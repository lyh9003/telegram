...
- 짧은 글: 3~5개 항목, 긴 글: 최대 20개 항목
- ~했음, ~함, ~였음, ~임, ~있음, 또는 단어맺음 식 간결체 사용
- 각 항목 사이 줄바꿈(\n) 포함
- 글이 길수록 요약도 길게 함


감성 분류 기준 (삼성전자 메모리 입장 기준):
- 긍정적: AI/반도체 수요 증가, 설비 투자 확대, 실적 개선, 공급 부족(수요 강세), 메모리 가격 상승, GPU 수요 급증, 데이터센터 투자 확대, 메모리 경쟁우위, 빅테크 투자확대
- 부정적: 반도체 수요 감소, 반도체 재고 증가, 규제 강화, 빅테크 실적 악화, 반도체 실적 악화, 메모리 가격 하락, 감산, 수출 제한, 빅테크 투자축소
- 중립적: 단순 현황 보고, 인사/조직 변경, 방향 불명확한 내용
※ 수요, 공급망 불안정·과부하 등 부정 신호이나 메모리 수요 강세로 연결될 시 긍정적으로 분류
